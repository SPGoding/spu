...
import { getNbtCompound, getNbtList, isNumeric, UpdateResult } from "../../utils/utils";
...
export class UpdaterTo19 {
    ...
    public static getResultMap(cmd: string, spus: string) {
        ...
        while (spusArg !== '' && begin < cmdSplited.length) {
            ...
            if (spusArg[0] === '%') {
                map.set(`%${cnt++}`, UpdaterTo19.upArgument(cmdArg, spusArg))
            }
    ...
    private static upCommand(input: string) {
        ...
        for (const spusOld of Spuses.pairs.keys()) {
            let map = UpdaterTo19.getResultMap(input, spusOld)
            if (map) {
    ...
    private static upArgument(arg: string, spus: string) {
        switch (spus.slice(1)) {
            case 'block_nbt':
                return UpdaterTo19.upBlockNbt(arg)
            case 'bool':
                return arg
            case 'command':
                return UpdaterTo19.upCommand(arg)
            case 'entity':
                return arg
            case 'entity_nbt':
                return UpdaterTo19.upEntityNbt(arg)
            case 'entity_type':
                return arg
            case 'item_nbt':
                return UpdaterTo19.upItemNbt(arg)
            case 'item_tag_nbt':
                return UpdaterTo19.upItemTagNbt(arg)
            case 'json':
                return UpdaterTo19.upJson(arg)
            case 'literal':
                return arg
            case 'num':
                return arg
    ...
    private static upBlockNbt(input: string) {
                ...
                spawnPotentials.forEach((potential: NbtValue) => {
                    if (potential instanceof NbtCompound) {
                        ...
                        if (entity instanceof NbtCompound) {
                            entity = getNbtCompound(UpdaterTo19.upEntityNbt(entity.toString()))
        ...
        /* SpawnData */ {
            ...
            if (spawnData instanceof NbtCompound) {
                spawnData = getNbtCompound(UpdaterTo19.upEntityNbt(spawnData.toString()))
    ...
    private static upEntityNbt(input_nbt: string) {
                    ...
                    recipes.forEach((v: NbtValue) => {
                        if (v instanceof NbtCompound) {
                            ...
                            if (buy instanceof NbtCompound) {
                                buy = getNbtCompound(UpdaterTo19.upItemNbt(buy.toString()))
                                v.set('buy', buy)
                            }
                            if (buyB instanceof NbtCompound) {
                                buyB = getNbtCompound(UpdaterTo19.upItemNbt(buyB.toString()))
                                v.set('buyB', buyB)
                            }
                            if (sell instanceof NbtCompound) {
                                sell = getNbtCompound(UpdaterTo19.upItemNbt(sell.toString()))
                                v.set('sell', sell)
                            }
                        }
                    })
        ...
        /* Items */ {
            ...
            if (items instanceof NbtList) {
                for (let i = 0; i < items.length; i++) {
                    ...
                    item = getNbtCompound(UpdaterTo19.upItemNbt(item.toString()))
                    items.set(i, item)
                }
            }
        }
        ...
        /* SelectedItem */ {
            ...
            if (selectedItem instanceof NbtCompound) {
                selectedItem = getNbtCompound(UpdaterTo19.upItemNbt(selectedItem.toString()))
        ...
        /* FireworksItem */ {
            ...
            if (fireworksItem instanceof NbtCompound) {
                fireworksItem = getNbtCompound(UpdaterTo19.upItemNbt(fireworksItem.toString()))
        ...
        /* TileID & TileEntityData */ {
            ...
            if (tileEntityData instanceof NbtCompound) {
                tileEntityData = getNbtCompound(UpdaterTo19.upBlockNbt(tileEntityData.toString()))
        ...
        /* Command */ {
            ...
            if (command instanceof NbtString) {
                command.set(UpdaterTo19.upCommand(command.get()))
            }
        }
        return nbt.toString()
    }
    ...
    private static upItemNbt(input: string) {
        ...
        /* tag */ {
            ...
            if (tag instanceof NbtCompound) {
                tag = getNbtCompound(UpdaterTo19.upItemTagNbt(tag.toString()))
                nbt.set('tag', tag)
            }
        }
        /* id */ {
    ...
    private static upItemTagNbt(input: string) {
        ...
        /* EntityTag */ {
            ...
            if (entityTag instanceof NbtCompound) {
                entityTag = getNbtCompound(UpdaterTo19.upEntityNbt(entityTag.toString()))
        ...
        /* BlockEntityTag */ {
            ...
            if (blockEntityTag instanceof NbtCompound) {
                blockEntityTag = getNbtCompound(UpdaterTo19.upBlockNbt(blockEntityTag.toString()))
    ...
    private static upJson(input: string) {
        if (input.slice(0, 1) === '"' || isNumeric(input) || Checker.isBool(input)) {
            ...
            let result: string[] = []
            for (const i of json) {
                result.push(UpdaterTo19.upJson(JSON.stringify(i)))
            }
            return `[${result.join()}]`
        } else {
            ...
            if (
                ...
                json.clickEvent.value = UpdaterTo19.upCommand(json.clickEvent.value)
            }

            if (json.extra) {
                json.extra = JSON.parse(UpdaterTo19.upJson(JSON.stringify(json.extra)))
            }
