...
export class MinecraftBlockPosParser implements ArgumentParser {
    ...
    public canParse(splited: string[], index: number): number {
        if (isNumeric(splited[index]) && isNumeric(splited[index + 1]) && isNumeric(splited[index + 2])) {
            return 3
        } else {
            return 0
        }
...
export class MinecraftColorParser implements ArgumentParser {
    ...
    public canParse(splited: string[], index: number): number {
        if (['black', 'dark_blue', 'dark_green', 'dark_aqua', 'dark_red', 'dark_purple', 'gold', 'gray', 'dark_gray',
            'blue', 'green', 'aqua', 'red', 'light_purple', 'yellow', 'white'].indexOf(splited[index]) !== -1) {
            return 1
        } else {
            return 0
        }
...
export class MinecraftItemSlotParser implements ArgumentParser {
    ...
    public canParse(splited: string[], index: number): number {
        if (['armor.chest', 'armor.feet', 'armor.head', 'armor.legs', 'weapon.mainhand', 'weapon.offhand',
            'container.0', 'container.1', 'container.2', 'container.3', 'container.4', 'container.5', 'container.6',
            'container.7', 'container.8', 'container.9', 'container.10', 'container.11', 'container.12', 'container.13',
            'container.14', 'container.15', 'container.16', 'container.17', 'container.18', 'container.19',
            'container.20', 'container.21', 'container.22', 'container.23', 'container.24', 'container.25',
            ...
            'container.38', 'container.39', 'container.40', 'container.41', 'container.42', 'container.43',
            'container.44', 'container.45', 'container.46', 'container.47', 'container.48', 'container.49',
            'container.50', 'container.51', 'container.52', 'container.53', 'enderchest.0', 'enderchest.1',
            'enderchest.2', 'enderchest.3', 'enderchest.4', 'enderchest.5', 'enderchest.6', 'enderchest.7',
            'enderchest.8', 'enderchest.9', 'enderchest.10', 'enderchest.11', 'enderchest.12', 'enderchest.13',
            'enderchest.14', 'enderchest.15', 'enderchest.16', 'enderchest.17', 'enderchest.18',
            'enderchest.19', 'enderchest.20', 'enderchest.21', 'enderchest.22', 'enderchest.23',
            'enderchest.24', 'enderchest.25', 'enderchest.26', 'inventory.0', 'inventory.1', 'inventory.2',
            'inventory.3', 'inventory.4', 'inventory.5', 'inventory.6', 'inventory.7', 'inventory.8',
            'inventory.9', 'inventory.10', 'inventory.11', 'inventory.12', 'inventory.13', 'inventory.14',
            'inventory.15', 'inventory.16', 'inventory.17', 'inventory.18', 'inventory.19',
            'inventory.20', 'inventory.21', 'inventory.22', 'inventory.23', 'inventory.24',
            'inventory.25', 'inventory.26', 'hotbar.0', 'hotbar.1', 'hotbar.2', 'hotbar.3', 'hotbar.4',
            'hotbar.5', 'hotbar.6', 'hotbar.7', 'hotbar.8', 'horse.saddle', 'horse.chest',
            'horse.armor', 'horse.0', 'horse.1', 'horse.2', 'horse.3', 'horse.4', 'horse.5',
            'horse.6', 'horse.7', 'horse.8', 'horse.9', 'horse.10', 'horse.11', 'horse.12',
            'horse.13', 'horse.14', 'villager.0', 'villager.1', 'villager.2', 'villager.3',
            'villager.4', 'villager.5', 'villager.6', 'villager.7'].indexOf(splited[index]) !== -1) {
            return 1
        } else {
            return 0
        }
    ...
    public canParse(splited: string[], index: number): number {
        if (ResourceLocation.test(splited[index])) {
            if (['dust', 'minecraft:dust'].indexOf(splited[index]) !== -1) {
                if (isNumeric(splited[index + 1]) && isNumeric(splited[index + 2]) &&
                    isNumeric(splited[index + 3]) && isNumeric(splited[index + 4]) &&
                    parseFloat(splited[index + 1]) >= 0 && parseFloat(splited[index + 1]) <= 1 &&
                    parseFloat(splited[index + 2]) >= 0 && parseFloat(splited[index + 2]) <= 1 &&
                    parseFloat(splited[index + 3]) >= 0 && parseFloat(splited[index + 3]) <= 1 &&
                    parseFloat(splited[index + 4]) >= 0 && parseFloat(splited[index + 4]) <= 1) {
                    return 5
                } else {
                    return 0
                }
...
export class MinecraftScoreboardSlotParser implements ArgumentParser {
    ...
    public canParse(splited: string[], index: number): number {
        if (['list', 'sidebar', 'belowName', 'sidebar.team.black', 'sidebar.team.dark_blue', 'sidebar.team.dark_green',
            'sidebar.team.dark_aqua', 'sidebar.team.dark_red', 'sidebar.team.dark_purple', 'sidebar.team.gold',
            'sidebar.team.gray', 'sidebar.team.dark_gray', 'sidebar.team.blue', 'sidebar.team.green',
            'sidebar.team.aqua', 'sidebar.team.red', 'sidebar.team.light_purple', 'sidebar.team.yellow',
            'sidebar.team.white'].indexOf(splited[index]) !== -1) {
            return 1
        } else {
            return 0
        }
