...
class ArgumentParser {
    constructor() {
        this.ResourceLocation = /^([a-zA-Z0-9\_\-\.]+:)?([a-zA-Z0-9\_\-\.]+\/)*[a-zA-Z0-9\_\-\.]+$/;
    ...
    parseBrigadierInteger(splited, index, min, max, acceptsAsterisk) {
        if (utils_1.isNumeric(splited[index])) {
    ...
    parseMinecraftEntity(splited, index, amount, type) {
        ...
        let exception;
        try {
            const sel = new selector_1.TargetSelector(join);
