...
class StdItem1_12 {
    constructor(name, data, tag, count, slot) {
...
exports.StdItem1_12 = StdItem1_12;
class StdItem1_13 {
    constructor(name, nbt, count, slot) {
        name = utils_1.completeNamespace(name);
        this.name = name;
        this.tag = nbt;
        this.count = count;
...
exports.StdItem1_13 = StdItem1_13;
/**
 * Providing a map storing old item IDs and new item IDs.
 */
class Items {
    static std112(id, name, data, tag, nbt) {
        let ansName;
        let ansData;
        let ansTag;
        let ansCount;
        ...
        if (id && !name && !nbt) {
            ansData = data ? data : 0;
            ansTag = utils_1.getNbtCompound(tag ? tag : '{}');
            ...
            const nbtC = utils_1.getNbtCompound(nbt);
            ...
            throw `Argument Error! Used ${id ? 'id, ' : ''}${name ? 'name, ' : ''}${data ? 'data, ' : ''}${tag && tag !== '{}' ? 'tag, ' : ''}${nbt && nbt !== '{}' ? 'nbt, ' : ''}.`;
        }
        ansName = utils_1.completeNamespace(ansName);
    ...
    static to113(std) {
        ...
        if (Items.DamagableItems.indexOf(ansName) !== -1 && data !== 0) {
            ansTag.set('Damage', new nbt_1.NbtInt(data));
        }
        else if (Items.MapItems.indexOf(ansName) !== -1 && data !== 0) {
            ansTag.set('map', new nbt_1.NbtInt(data));
        }
        else {
        ...
        /* EntityTag */ {
            if (ansName === 'minecraft:bat_spawn_egg') {
                ...
                if (entityTag instanceof nbt_1.NbtCompound) {
                    ...
                    if (id instanceof nbt_1.NbtString) {
                        const after = entities_1.default.to113(id.get());
    ...
    static toNominalColor(input, suffix) {
...
Items.NumericID112_NominalID112 = [
    [0, 'minecraft:air'],
    [1, 'minecraft:stone'],
    [2, 'minecraft:grass'],
    [3, 'minecraft:dirt'],
...
Items.MapItems = ['minecraft:map', 'minecraft:filled_map'];
Items.SpawnEgges = [
